...
| 5 | **HARDENING Phase 2.2** — real-DAL backend integration tests (L) | HARDENING | ⏳ search half #246; routes + contract round-trip 2026-06-10; `adminOperations` #441. Remaining: mocked-test sweep (`subscription` is the last candidate that would gain a real table) + CI `--integration` wiring — **the wiring now gates the value of all 14 real-DAL suites, none of which CI runs today** |
| 6 | **BUILD Phase 1** — requirements-first Build page (motion/stroke/speed/payload/orientation form → motion-system kit) | BUILD | 🚧 PR 1A shipped end-to-end: `compatible_actuators` (#247), `/api/v1/relations/actuators` on Express (#262), `_distribution_position` block (`relations.ts:attachStrokeDistributionPositions`), and the v2 (FastAPI) mirror (#440 — route + `relations.stroke_distribution_positions`, which is now the source of truth for the badge ranker). Next: 1B strip-down → 1C page → 1D redirect |
