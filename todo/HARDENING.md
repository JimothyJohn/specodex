...
- **Follow-up (found by the migration above, production change so out of scope for a test PR):** `POST /api/upload` validates `product_name` / `manufacturer` / `product_type` / `filename` for truthiness only, never for type. A non-string `product_type` becomes a 500 instead of a 400, and a non-string `filename` would take the same path through `filename.toLowerCase()`. Fix is a type guard in `app/backend/src/routes/upload.ts` plus flipping the two pinned expectations (mocked sibling + real-DAL). Low severity — attacker-controlled 500 on the caller's own request, no write occurs — but it is a validation gap, not intended behaviour.
- The remaining ~10 mocked tests (step 5). Each migration is mechanical given the foundation, but each test needs its own seed/cleanup so the diff is non-trivial — better split across PRs.
